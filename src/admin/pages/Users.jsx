          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-black/5">
                ...
                <Td>
                  {r.is_verified ? (
                    <button className="btn-outline" onClick={() => toggle(r.id, false)}>Revoke</button>
                  ) : (
                    <button className="btn-primary" onClick={() => toggle(r.id, true)}>Approve</button>
                  )}
                </Td>
              </tr>
            ))}
...
const Th = (p) => <th className={`px-3 py-2 ${p.className||""}`}>{p.children}</th>;
const Td = (p) => <td className={`px-3 py-2 ${p.className||""}`}>{p.children}</td>;
